...
setwd("~/Library/CloudStorage/OneDrive-UniversityofGlasgow/PhD/AIM 1/pipiens-ISDM")
...
# rename grid_id column from grid_id_area2km, to grid_id
names(z_climate_data)[1] <- "grid_id"
...
z_temp_min_clean <- z_temp_min
z_temp_min_clean[is.na(z_temp_min_clean)] <- 0
...
z_rain_clean <- z_rain
z_rain_clean[is.na(z_rain_clean)] <- 0

z_rain2 <- z_rain_clean^2


stopifnot(
  all(dim(z_rain2) == dim(z_rain)),
  all(is.finite(z_rain2))
)
...
names(z_land_data)[1] <- "grid_id" # rename grid_id column from grid_id_area2km, to grid_id
...
# survey_df <- survey_df %>%
#   left_join(z_land_data, by = c("grid_id" = "grid_id"))
# 
...
# survey_df <- survey_df %>%
#   left_join(z_climate_data, by = c("grid_id" = "grid_id", "date" = "date"))
# 
...
z_temp_clean <- z_temp
z_temp_clean[is.na(z_temp_clean)] <- 0
...
# =============================================================
par(mfrow = c(1, 2))
hist(z_rain, breaks = 40, main = "z_rain", xlab = "")
