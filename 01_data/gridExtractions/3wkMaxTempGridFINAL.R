...
grid_vect <- vect(grid_1km) # Convert sf grid to SpatVector
...
load_nc_temp_data <- function(nc_dir, var_name = "tasmax") {
...
filter_nc_files_for_temp_range <- function(nc_dir, start_date, end_date) {
  nc_files <- list.files(nc_dir, pattern = "tasmax_hadukgrid_uk_1km_day_.*\\.nc$", full.names = TRUE)
...
extract_grid_temp_ts_optimized <- function(grid_df, dates_to_extract, nc_dir, n_cores = NULL) {
  ...
  results <- future_lapply(dates_to_extract, function(target_date) {
    ...
    data.frame(
      ...
      x_bng = grid_df$x_bng,
      y_bng = grid_df$y_bng,
      mean_max_temp_21d_celsius = round(temp_values, 1),  # changed to 21d
...
raw_values <- ncvar_get(nc, "tasmax")[test_idx[1], test_idx[2], ]
...
#---- checks ------
table(unique(temp_grid_ts$grid_id) %in% unique(grid_clipped$grid_id))

length(unique(temp_grid_ts$grid_id))

missing_grids <- setdiff(grid_clipped$grid_id, temp_grid_ts$grid_id) # ...

original_ids <- unique(grid_clipped$grid_id)
result_ids <- unique(temp_grid_ts$grid_id)
...
coord_check <- merge(
  original_coords,
  unique(temp_grid_ts[, c("grid_id", "x_bng", "y_bng")]),
  by = "grid_id"
)
