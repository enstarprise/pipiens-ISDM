...
z_climate_data <- z_climate_data %>%
  mutate(
    grid_id_1 = grid_id_lookup[as.character(grid_id_area2km)],
...
n_land_covs <- ncol(z_land_data[, setdiff(grep("^z_", colnames(z_land_data), value = TRUE), 
                                          c("z_urban_km2", "z_suburban_km2", "z_poi_count_grouped", "z_reports"))])
...
saveRDS(z_temp, "01_data/env_temp_data.rds")

save(
  z_rain, 
  file = "01_data/env_rain_data.RData"
)
saveRDS(z_rain, "01_data/env_rain_data.rds")

# Save with maximum compression
saveRDS(z_temp, "01_data/env_temp_data.rds", compress = "xz")
save(z_temp, file = "01_data/env_temp_data.RData", compress = "xz")
