...
z_temp_min_subset <- z_temp_min[obs_grid_positions, , drop = FALSE]
z_rain_subset <- z_rain[obs_grid_positions, , drop = FALSE]

cat(sprintf("  Climate matrices reduced to: %d × %d\n", 
            nrow(z_temp_min_subset), ncol(z_temp_min_subset)))
...
z_poi_subset <- z_land_data %>%
  filter(grid_id_area2km %in% observed_grid_ids) %>%
  arrange(grid_id_area2km) %>%
  pull(z_poi_count_grouped)

z_reports_subset <- z_land_data %>%
  filter(grid_id_area2km %in% observed_grid_ids) %>%
  arrange(grid_id_area2km) %>%
  pull(z_reports)

z_buildings_subset <- z_land_data %>%
  filter(grid_id_area2km %in% observed_grid_ids) %>%
  arrange(grid_id_area2km) %>%
...
z_temp_min_inits <- impute_missing_normally(z_temp_min_subset, verbose = TRUE)
...
model_code <- nimbleCode({
  ...
  ## ECOLOGICAL PROCESS - Now only for observed grids
  for(g in 1:n_grids) {
    logit_p[g] <- delta0 + delta_poi * z_poi[g] + delta_reports * z_reports[g]
  ...
  ## Background (normalizing constant)
  for(t in 1:n_dates) {
    background[t] <- inprod(lambda_thinned[1:n_grids, t], 
                            rep(1, n_grids)) / n_obs_po
  }
