...
// Fixing the rain imputation, with heavier tailed rain imputation
// saved file is stan_fit_marginalized_1km_impute_fix
...
functions {
  ...
  real binomial_negbin_marginal_lpmf(int y_obs, real p_detect, 
    ...
    for (n in 1:n_components) {
      ...
      // Early stopping: if past observation and contribution is tiny, stop
...
parameters {
  ...
  real<lower=1e-6, upper=5> sigma_rain;      // ADDED UPPER BOUND
  real<lower=1e-6, upper=5> sigma_rain_time; // ADDED UPPER BOUND
  
  
  // added degrees of freedom params
  real<lower=2, upper=30> nu_rain;  // degrees of freedom for student_t // prev. real<lower=2> nu_rain;
...
transformed parameters {
  ...
  vector[n_dates] gamma_rain_time = gamma_rain_time_raw - mean(gamma_rain_time_raw); // REPLACED vector[n_dates] gamma_rain_time = gamma_rain_time_raw - mean(gamma_rain_time_raw);
...
model {
  ...
  nu_rain ~ gamma(2, 0.5);  // Prior on degrees of freedom (mean = 4)
  ...
  // Rain: observed values (CHANGE TO STUDENT-T)
  for (g in 1:n_grids_total) {
   for (t in 1:n_dates) {
     if (rain_missing[g, t] == 0) {
        z_rain_obs[g, t] ~ student_t(nu_rain, mu_rain[g, t], sigma_rain);
     }
    }
  }
  
  // Prior on imputed values
  {
   int temp_idx = 1;
   int rain_idx = 1;
    ...
    for (g in 1:n_grids_total) {
     for (t in 1:n_dates) {
      ...
      if (rain_missing[g, t] == 1) {
        z_rain_imputed_raw[rain_idx] ~ student_t(nu_rain, mu_rain[g, t], sigma_rain);
        rain_idx += 1;
        }
      }
    }
...
generated quantities {
  ...
  // -------------------------------------------------
  // POSTERIOR PREDICTICE REPS FOR IMPUTED VALUES
    // Posterior predictive reps for imputed values
  // -------------------------------------------------
  matrix[n_grids_total, n_dates] z_temp_rep;
  matrix[n_grids_total, n_dates] z_rain_rep;
  for (g in 1:n_grids_total) {
    for (t in 1:n_dates) {
      z_temp_rep[g, t] = normal_rng(mu_temp[g, t], sigma_temp);
      z_rain_rep[g, t] = student_t_rng(nu_rain, mu_rain[g, t], sigma_rain);
      }
  }
