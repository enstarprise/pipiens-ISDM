...
functions {
  // function formarginalized log-likelihood for one observation
  // sums over N = y_obs to N_max
  real binomial_negbin_marginal_lpmf(int y_obs, real p_detect, real lambda, real phi, int N_max) {
    // store the log prob of each possible latent N; with length where N 
    // must be >= to y_obs, and the max is N_max 
    vector[N_max - y_obs + 1] log_components;
    
    // loop over latent N values
    for (n in 1:(N_max - y_obs + 1)) {
      int N = y_obs + n - 1;
      
      // a binom dist observation model: log prob of (y_obs given N and p_detect)
      //                                      + 
      // a NB2 dist state model: log prob of (N given lambda and phi dispersion) 
      log_components[n] = binomial_lpmf(y_obs | N, p_detect) + 
                          neg_binomial_2_lpmf(N | lambda, phi);
    }
    
    // the marginalisation using the log sum exp trick...
...
model {
  ...
  // MARGINALIZED LIKELIHOOD: just for the survey observations since it has the latent discrete...
